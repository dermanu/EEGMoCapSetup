	...
	else 
	{
		try 
		{
			...
			else
			{
				...
				//Super jank home made
				//auto freq_val = ui->sbSyncFreq->value();
				//auto jank = [](t_output_mode, freq_val, pulse_w) {
				//	std::this_thread::sleep_for(std::chrono::seconds(10)); //Sleeping for 10 seconds to allow for getting the steram before it pushes to outlet
				//	m_pLiveAmp->setOutTriggerMode(t_output_mode, 0, freq_val, pulse_w);
				//};

				//new std::thread(jank, m_TriggerOutputMode, freq_val, nPulseWidth);
				//Super jank home made end
				...
				*/
				...
				this->setWindowTitle(("Streaming from LiveAmp asdfasdfd" + sSerialNumber).c_str() );

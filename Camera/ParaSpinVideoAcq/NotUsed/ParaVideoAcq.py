...
def capture(camNum):
        ...
        try:
            while True:                  ## Maybe only parallelly execute while loop
                ret, frame = cap.read()
...
if __name__ == "__main__":
    CamTotal = 4
    Process(target=capture, args=(0,)).start()
    Process(target=capture, args=(1,)).start()

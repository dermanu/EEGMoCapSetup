...
class VideoAcq(object):
    def __init__(self, camNum, start_time):
        ...
        self.buflen = 200
        self.buffer = [[[0 for k in range(self.buflen)] for j in range(settings['width'])] for i in range(settings['height'])]
    ...
    # Save frame
    def save_Frame(self):
        if len(self.buffer[0][0]) == self.buflen:
            for f in range(self.buflen):
                self.writer.write(self.buffer[][][f])
            self.buffer = []
